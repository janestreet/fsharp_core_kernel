...
module Controllable =
  type t (start_time : System.DateTime) =
    ...
    member this.advance_immediately_by span =
      Sequencer.with_ now (fun now -> advance_to (!now + span) now)
    ...
    interface time_source with
      member this.now() = Sequencer.with_ now (fun now -> !now)

      member this.sleep_for span =
        Sequencer.with_
          now
          (fun now ->
            let sleep_until = !now + span

            Auto_reset_event.set sleep_signal

            while !now < sleep_until do
